    ...
    <div class="conttent_bk" id="product">
      ...
      <section class="comment">
        ...
        <div class="comment_contentAll">
          ...
          <div v-if="proComments.length==0" class="noPeocomments">此商品暂时还没有评论！</div>
        </div>
      </section>
      <footer>
      </footer>
...
//          console.log(resData.data[0])
          resData.data[0]['pro_storetime']=_this.formatDate(new Date( resData.data[0]['pro_storetime']))
...
//          console.log(_this.resData);
        ...
        this.$axios.post('http://localhost:9999/productComments.do', postdata).then(function (resDatac) {
          console.log(resDatac.data)
          for(let i = 0;i<resDatac.data.length;i++){
          let str = resDatac.data[i]["com_message_date"].substring(0, resDatac.data[i]["com_message_date"].length - 5);
            resDatac.data[i]["com_message_date"] = str.split('T').toString().replace(/,/, ' ');
          }
